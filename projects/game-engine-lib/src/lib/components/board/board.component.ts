...
export class BoardComponent implements OnInit {
  ...
  initBoard() {
    const _ctx = this.canvas.nativeElement.getContext("2d");
    if (_ctx) {
      this.ctx = _ctx;
  ...
  initNext() {
    const _ctxNext = this.canvasNext.nativeElement.getContext("2d");
    if (_ctxNext) {
      this.ctxNext = _ctxNext;
  ...
  resetGame() {
    ...
    this.time = { start: 0, elapsed: 0, level: +LEVEL[this.level as keyof typeof LEVEL] };
    this.paused = false;
    this.addOutlines();
  }
